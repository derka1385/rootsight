...
type Props = {
  ...
  onOpenSample: (p: PlantProfile) => void;
  onScan: () => void;
};
...
export default function Home({ name, reminders, myPlants, onOpenSaved, onOpenSample, onScan }: Props) {
  ...
  const species = new Set(plants.map((p) => p.profile.species.scientificName)).size;
    ...
    <div className="screen">
      ...
      {reminders && thirsty.length > 0 && (
        <div className="alert" role="status">
          <DropIcon />
          <p>{thirsty.length === 1 ? `${thirsty[0].profile.species.commonName} is thirsty today` : `${thirsty.length} plants are thirsty today`}</p>
      ...
      {plants.length === 0 ? (
        ...
        <div className="grid2">
          {plants.map((p) => {
            ...
            const stage = stageOf(p.profile, state);
            const days = daysUntilWater(p);
            return (
              <button key={p.id} className="plant-card" onClick={() => onOpenSaved(p)}>
                <div className="thumb"><PlantThumb profile={p.profile} /></div>
                <div className="stack" style={{ gap: 6 }}>
                  <h3>{p.profile.species.commonName}</h3>
      ...
      <div className="samples">
        {Object.values(fixtures).map((p) => (
          <button key={p.species.scientificName} className="sample" onClick={() => onOpenSample(p)}>
            <div className="thumb"><PlantThumb profile={p} /></div>
            <strong style={{ fontSize: 14 }}>{p.species.commonName}</strong>
            <span className="small muted">{p.wiki.difficulty} to grow</span>
          </button>
        ))}
      </div>
    </div>
